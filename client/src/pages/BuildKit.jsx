...
export default function BuildKit() {
  ...
  async function fetchProduct(){
    const data = await (await axios.get(`https://maddkit.com/wp-json/wc/v3/products?category=${categoryId[kitStep-1]}&per_page=50&consumer_key=ck_093af7accbe95ac38eadfed5c75e3e9b3baa82e6&consumer_secret=cs_97b91a6da87365fe251f05434dba14a10c02a009`)).data;
    setProducts(data);
  }
        ...
        {/* {kitStep === 1 ?
          ...
          kitStep === 2 ?
            <>
              ...
              <Card img={'img10.jpg'} price={999} ogprice={1899} title={'Better Together Neon Sign – Gold Color LED Wall Decor for Weddings'} off={47} />
              <Card img={'img9.jpg'} price={999} ogprice={1899} title={'Happy Birthday Neon Sign – LED Wall Decor with Adapter & Hanging'} off={47} />
              <Card img={'img10.jpg'} price={999} ogprice={1899} title={'Better Together Neon Sign – Gold Color LED Wall Decor for Weddings'} off={47} />
              ...
              <>
                ...
                <Card img={'img12.jpg'} price={149} ogprice={399} title={'Bride to Be Banner – Bachelorette Party Decoration with Purple '} off={63} />
                <Card img={'img11.webp'} price={179} ogprice={399} title={'Glitter Fishtail Happy Birthday Banner (Gold / Silver / Pink) – Party Decoration'} off={55} />
                <Card img={'img12.jpg'} price={149} ogprice={399} title={'Bride to Be Banner – Bachelorette Party Decoration with Purple'} off={63} />
...
{products.length!==0?products.map((item, index)=>(
  ...
  <Card key={index} img={item.images[0]?.src} toast={handleToast} price={item.price} ogprice={item.regular_price} title={item.name} off={Math.round(((Number(item.regular_price) - Number(item.price) )/Number(item.regular_price)) * 100 )} id={item.id} variations={item.variations} category={category[kitStep - 1]}/>
